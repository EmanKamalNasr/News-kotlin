...
class FilmFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val call: Call<News>
        call = newsApiInterface!!.getNews(API_KEY, getString(R.string.sec_film))
        call.enqueue(object : Callback<News> {
            override fun onResponse(call: Call<News>, response: Response<News>) {
                if (response.isSuccessful && response.body()!!.response!!.articles != null) {
                    ...
                    val filmRecyclerView = view.findViewById<View>(R.id.home_recyclerview) as RecyclerView
                    filmRecyclerView.layoutManager = LinearLayoutManager(activity)
                    filmRecyclerView.adapter = newsAdapter
                } else {
