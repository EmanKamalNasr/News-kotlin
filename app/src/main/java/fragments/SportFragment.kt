...
class SportFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val call: Call<News>
        call = newsApiInterface!!.getNews(API_KEY, getString(R.string.sec_sport))
        call.enqueue(object : Callback<News> {
            override fun onResponse(call: Call<News>, response: Response<News>) {
                if (response.isSuccessful && response.body()!!.response!!.articles != null) {
                    ...
                    val sportRecyclerView = view.findViewById<View>(R.id.home_recyclerview) as RecyclerView
                    sportRecyclerView.layoutManager = LinearLayoutManager(activity)
                    sportRecyclerView.adapter = newsAdapter
                } else {
