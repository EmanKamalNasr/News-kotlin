...
class MusicFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val call: Call<News>
        call = newsApiInterface!!.getNews(API_KEY, getString(R.string.sec_music))
        call.enqueue(object : Callback<News> {
            override fun onResponse(call: Call<News>, response: Response<News>) {
                if (response.isSuccessful && response.body()!!.response!!.articles != null) {
                    ...
                    val musicRecyclerView = view.findViewById<View>(R.id.home_recyclerview) as RecyclerView
                    musicRecyclerView.layoutManager = LinearLayoutManager(activity)
                    musicRecyclerView.adapter = newsAdapter
